...
Namespace CollapsiblePanel
    Public Class CollapsePanel
        ...
        Private Dim     components      As  Container
        ' array of CollapseGroupBoxes - a panel can contain multiple group boxes
        Private Dim     groupBoxArray   As  ArrayList
        ...
        '/</summary>
		Public  Sub Add(ByVal groupBox As CollapseGroupBox)
		...
		Private  Sub CollapseBox_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Dim groupBox As CollapseGroupBox = _
			    CType(groupBoxArray(nIndex), CollapseGroupBox)
 
            ' compute the difference between the full height and collapsed height
			Dim nDelta As Integer
			If groupBox.Height = CollapseGroupBox.intCollapsedHeight
				nDelta = -(groupBox.FullHeight - CollapseGroupBox.intCollapsedHeight)
			Else 
				nDelta = (groupBox.FullHeight - CollapseGroupBox.intCollapsedHeight)
			End If
			...
			For  i  = (nIndex + 1) To groupBoxArray.Count - 1 Step 1
				groupBox = CType(groupBoxArray(i), CollapseGroupBox)
